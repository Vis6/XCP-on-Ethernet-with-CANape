...
void psLEDInit()
{
	...
	Status = XGpioPs_CfgInitialize(&PS_Gpio, PS_GPIO_Config, PS_GPIO_Config->BaseAddr);
	if(Status != XST_SUCCESS)
		xil_printf("PS MIO GPIO Initialize failed!\n");
	else
		xil_printf("PS MIO GPIO Initialize succeed!\n");

	XGpioPs_SetDirectionPin(&PS_Gpio, OUTPUT_PORT, 1);
	XGpioPs_SetOutputEnablePin(&PS_Gpio, OUTPUT_PORT, 1);
...
int main( void )
{
	...
	for more details. */
	xil_printf("Error occurs! RTOS system doesn't start correctly!\r\n");
	for( ;; );
}
/*-----------------------------------------------------------*/
